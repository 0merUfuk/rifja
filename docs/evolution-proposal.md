...
- v2 (optional, explicit): a small allowlist of safe operations already CLI-exposed (`refresh`, `doctor`) behind POST + session + a **session-bound CSRF token** whose validation fails closed on every state-changing request: token required in the body or a custom header, exact `Origin` match against the dashboard origin, `Sec-Fetch-Site: same-origin` required, and any missing or mismatched value rejected — with negative tests for another loopback port, missing headers, and invalid tokens. `SameSite=strict` does not cover same-site, cross-origin requests between different loopback ports, so the visible confirm is a UX control, not the CSRF boundary. Still no delete/export-to-disk from the browser.
