...
| Daily aggregation meets the warm query budget without dropping selected evidence | app.daily; schema 3 covering index; schema 5 partial index on unresolved event times; correction-scoped replay kinds | large installed daily query (0.731s p95, schema 5); real-corpus correction-replay regression test in test_daily_selection.py; schema 2 upgrade and failure rollback in test_resilience — see [performance evidence](performance.md#schema-5-closing-the-daily-gate). **Still not met on a real store with many unregistered-project records**: `coverage()`'s `unassociated_records` count dominates independently of the above — see [0.4.2 finding](performance.md#042-real-corpus-correction-replay), not yet closed. |
...
| Schema 4->5 upgrade (partial index on unresolved event times) preserves existing data and migrates automatically with a pre-migration backup | store.py schema migration | test_resilience.py schema version/migration assertions |
