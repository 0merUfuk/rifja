...
| Daily aggregation meets the warm query budget without dropping selected evidence | app.daily; schema 3 covering index; schema 5 partial index on unresolved event times | large installed daily query (0.731s p95, schema 5); schema 2 upgrade and failure rollback in test_resilience — see [performance evidence](performance.md#schema-5-closing-the-daily-gate) |
