...
`rifja ui` serves the management plane on `127.0.0.1` (default port 41970, overridable with `--port`; a taken port is an error, never a silent fallback). It prints a one-time sign-in URL: the first open exchanges the embedded token for an `HttpOnly` + `SameSite=strict` session cookie and invalidates the token, so the link cannot be replayed. Every page — static assets included — requires that session, responses carry `Cache-Control: no-store` and a restrictive CSP, and the browser is opened only with `--open`.

The dashboard is the operator's observability surface, not a usage point: its home screen is **Activity** — every tool call your agent made through Rifja (tool, summary, status, duration), newest first, failures kept and marked, live-updating. The other sections are **Overview** (state, 14-day record shape, doctor), **Sessions** and per-session evidence chains, **Search**, **Memory** (agent proposals awaiting your acceptance first), **Sources** (registration and import health, paginated) and **Settings** (read-only configuration view; changes go through your agent or the CLI). It is GET-only — there are no mutation endpoints — pages are bounded (keyset pagination, aggregate-first counts) and gzip-compressed, and transcript-derived content is escaped with no inline script. Stop with Ctrl-C; the server exists only while the command runs. The trust boundary is documented in [the threat model](rifja-threat-model.md).
